...
def createActionScript(ruleDict, CD):
    ...
    while (cdQueue != []):
        ...
        itemToExpand = cdQueue[0]
        del cdQueue[0]
        
        if itemToExpand[0] != '':
                actionScriptBook.append([itemToExpand[0]])
        
        if ruleDict.has_key(itemToExpand[1]):
            if len(actionScriptBook) == 0:
                actionScriptBook.append(itemToExpand[1])
                actionScriptBook.append('actor')
                actionScriptBook.append(not_known_formatter(itemToExpand[2]['actor']))
                
            elif isinstance(actionScriptBook[len(actionScriptBook)-1], list):
                actionScriptBook[len(actionScriptBook)-1].append(itemToExpand[1])
                actionScriptBook[len(actionScriptBook)-1].append('actor')
                actionScriptBook[len(actionScriptBook)-1].append(not_known_formatter(itemToExpand[2]['actor']))
            else:
                actionScriptBook.append(itemToExpand[1])
                actionScriptBook.append('actor')
                actionScriptBook.append(not_known_formatter(itemToExpand[2]['actor']))
            ...
            for field in ruleDict[itemToExpand[1]].getRelevantFields():
                print "field: ", field
                if itemToExpand[2].has_key(field):
                    if (isinstance(itemToExpand[2][field], dict)):
                        
                        cdQueue.append([field,itemToExpand[2][field].keys()[0], itemToExpand[2][field][itemToExpand[2][field].keys()[0]]])
                    else:
                        ...
                        if isinstance(actionScriptBook[len(actionScriptBook)-1], list):
                            ...
                            actionScriptBook[len(actionScriptBook)-1].append(not_known_formatter(itemToExpand[2][field]))
                        else:
                            actionScriptBook.append(field)
                            actionScriptBook.append(not_known_formatter(itemToExpand[2][field]))
